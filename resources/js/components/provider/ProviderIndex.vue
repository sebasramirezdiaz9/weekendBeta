<template>
    <div>
       <v-server-table :url="$route('provider.index')" :columns="columns" ref="table" :options="options" :filterByColumn="true" >
...
<script>
    ...
    import ProviderShow from './ProviderShow.vue';
        ...
        methods: {
            deleteRegister(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(route('provider.destroy', [id]), {
                        }).then( () => {
